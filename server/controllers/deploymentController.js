...
const mapPickup = p => ({
  tmoNo: p.tmoNo || undefined, // preserve existing; undefined triggers generation for new stops
...
const createDeployment = async (req, res, next) => {
  try {
    ...
    const newDeployment = await Deployment.create({
      pickups: pickups.map(mapPickup), // tmoNo generated by pre-save hook for each new stop
      truckId,
      driverId,
      truckType,
      helperCount,
...
const updateDeployment = async (req, res, next) => {
  try {
    ...
    // mapPickup preserves tmoNo for existing stops and leaves it undefined
    // for new ones so the pre-save hook generates a fresh tmoNo only for
    // stops that don't have one yet.
    ...
    // ── Auto-sum totalSacksCount and totalWeightKg from all pickup stops ──────
    // This runs after both Option A and Option B are applied, so it always
    // reflects the latest values across every stop.
    ...
    const updateObj = {
      ...
      // ── Always use computed totals derived from pickup stops ──────────────
    ...
    // ── Option A: log pickupIn / pickupOut for full array replacement ──────────
    if (pickups !== undefined && finalStatus !== 'canceled') {
      for (let i = 0; i < pickups.length; i++) {
        const p = pickups[i]
        // Match original stop by tmoNo if available, otherwise fall back to index
    ...
    // ── Option B: log pickupIn / pickupOut for per-stop patches ───────────────
    if (pickupUpdates && Array.isArray(pickupUpdates)) {
      for (const update of pickupUpdates) {
