...
const defaultContact = { contactPerson: "", contactPersonNo: "" };
...
function DeploymentDetailsModal({
  ...
  const addContact = () => {
    if ((editForm.receivingContacts?.length || 0) >= MAX_CONTACTS) return;
    setEditForm((prev) => ({
      ...prev,
      receivingContacts: [
        ...(prev.receivingContacts || []),
        { ...defaultContact },
      ],
    }));
  };
  ...
  const removeContact = (index) => {
    if ((editForm.receivingContacts?.length || 0) <= 1) return;
    setEditForm((prev) => ({
      ...prev,
    ...
    try {
      ...
      pickups.forEach((pickup, idx) => {
        ...
        ly = field(
          ...
          capitalizeWords(primaryContact.contactPerson),
          LEFT,
          ly,
          FW,
        );
        ly = field("Contact No", primaryContact.contactPersonNo, LEFT, ly, FW);
        ly = field(
          "No. of Sacks",
...
const DeploymentInfoTab = ({
  ...
  const receivingContacts =
    editForm?.receivingContacts && editForm.receivingContacts.length > 0
      ...
      : editForm?.receivingContactPerson
        ? [
            {
              contactPerson: editForm.receivingContactPerson || "",
              contactPersonNo: editForm.receivingContactPersonNo || "",
            },
          ]
        : [{ contactPerson: "", contactPersonNo: "" }];
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 gap-x-5 gap-y-4 border border-gray-200 rounded-xl p-5 max-sm:p-4">
          ...
          <div className="col-span-full">
            ...
            <div className="flex flex-col gap-2.5">
              {receivingContacts.map((contact, index) => (
                <div
                  key={index}
                ...
                >
                  <div className="col-span-2 flex items-center justify-between mb-0.5">
                    ...
                    {isEditMode && receivingContacts.length > 1 && (
                      <button
                        type="button"
                  ...
                  <div className="flex flex-col gap-1.5">
                    {isEditMode ? (
                      ...
                      <div className="flex items-center bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 shadow-sm min-h-[38px]">
...
const PickupSitesTab = ({
  ...
  useEffect(() => {
    if (pickups.length > prevLengthRef.current && lastStopRef.current) {
      lastStopRef.current.scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
    }
