...
export const exportDeploymentToExcel = async allDeployments => {
  ...
  worksheet.columns = [
    { width: 12 }, //  1 - DP Code
    { width: 12 }, //  2 - Plate No
    { width: 12 }, //  3 - Truck Type
    { width: 20 }, //  4 - Driver
    { width: 14 }, //  5 - TMO No.
    { width: 22 }, //  6 - Pickup Site
    { width: 18 }, //  7 - Municipality
    { width: 25 }, //  8 - Destination
    { width: 10 }, //  9 - Status
    { width: 18 }, // 10 - Departed
    { width: 18 }, // 11 - Pick-up In
    { width: 18 }, // 12 - Pick-up Out
    { width: 18 }, // 13 - Dest. Arrival
    { width: 18 }, // 14 - Dest. Departure
    { width: 12 }, // 15 - Unloading Time
    { width: 15 }, // 16 - Territory
    { width: 15 }, // 17 - Hybrid
    { width: 12 }, // 18 - Flagging
    { width: 20 }, // 19 - Flagging Remarks
    { width: 12 }, // 20 - Orig Plate No
    { width: 12 }, // 21 - Orig Truck Type
    { width: 20 }, // 22 - Orig Driver
    { width: 18 }, // 23 - Replaced At
    { width: 18 }, // 24 - Replacement Reason
    { width: 25 } // 25 - Replacement Remarks
  ]
  ...
  // ─── data rows ───────────────────────────────────────────────────────────────
  // NOTE: No merged cells — shared values are repeated on every pickup row so
  // the sheet can be converted to an Excel Table without any data loss.
  ...
  allDeployments.forEach(deployment => {
    ...
    // Shared values — written on EVERY pickup row (no merging)
    const sharedValues = {
    ...
    pickups.forEach(pickup => {
      const row = worksheet.addRow([
        sharedValues.dpCode, //  1
        sharedValues.plateNo, //  2
        sharedValues.truckType, //  3
        sharedValues.driver, //  4
        pickup.tmoNo || '', //  5 per pickup
        capitalizeWords(pickup.pickupSite || ''), //  6 per pickup
        capitalizeWords(pickup.municipality || ''), //  7 per pickup
        sharedValues.destination, //  8
        sharedValues.status, //  9
        sharedValues.departed, // 10
        isCanceled ? '' : formatDateTime(pickup.pickupIn || ''), // 11 per pickup
        isCanceled ? '' : formatDateTime(pickup.pickupOut || ''), // 12 per pickup
        sharedValues.destArrival, // 13
        sharedValues.destDeparture, // 14
        sharedValues.unloadingTime, // 15
        sharedValues.territory, // 16
        sharedValues.hybrid, // 17
        sharedValues.flagging, // 18
        sharedValues.flaggingRemarks, // 19
        sharedValues.origPlateNo, // 20
        sharedValues.origTruckType, // 21
        sharedValues.origDriver, // 22
        sharedValues.replacementDate, // 23
        sharedValues.replacementReason, // 24
        sharedValues.replacementRemarks // 25
      ])

      row.height = 20

      for (let col = 1; col <= 25; col++) {
        row.getCell(col).style = styles.data
      }
    })
  })
