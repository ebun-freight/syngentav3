...
// ─── Shared class tokens ─────────────────────────────────────────────────────
const CLS = {
  card: "bg-white p-4 sm:p-5 rounded-xl shadow-card3 sm:border sm:border-gray-100",
  cardOverflow:
    "bg-white rounded-xl shadow-card3 sm:border sm:border-gray-100 overflow-hidden",
  cardHeader: "mb-3 sm:mb-4",
  cardTitle: "text-xs sm:text-sm md:text-base font-semibold text-gray-900",
  cardSubtitle: "text-xxs sm:text-xs text-gray-500 mt-0.5",
  thCell:
    "px-3 sm:px-4 py-2 text-left text-xxs font-semibold text-gray-500 uppercase tracking-wider",
  tdCell: "px-3 sm:px-4 py-3 whitespace-nowrap text-xxs sm:text-xs",
  chartSm: "h-48 sm:h-56",
  chartMd: "h-56 sm:h-64",
  chartLine: "h-56 sm:h-72",
  chartLg: "h-72 sm:h-80",
  tabSection: "space-y-3 sm:space-y-5",
};
...
const Dashboard = () => {
  ...
  const fieldVsPlantOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        callbacks: {
          label: (ctx) =>
            `${ctx.dataset.label}: ${(ctx.raw || 0).toLocaleString()} kg`,
        },
      },
    },
    scales: {
  ...
  const getAllDriversBarData = () => {
    ...
    return {
      labels: sorted.map(
        (d) =>
          d.name?.replace(/\b\w/g, (c) => c.toUpperCase()) || "Unknown Driver",
      ),
      datasets: [
        {
  ...
  const getFlaggingDistributionPieData = () => {
    if (!analytics?.charts?.flaggingDistribution)
      return { labels: [], datasets: [] };
    const bgColors = analytics.charts.flaggingDistribution.labels.map(
      (label) => {
        const l = label.toLowerCase();
          ...
          <div className="space-y-1 min-w-0">
            <div className="flex items-center gap-2">
              <div className={`p-1.5 rounded-lg ${c.bg} shrink-0`}>
                <Icon className={`text-sm sm:text-base ${c.text}`} />
              </div>
              <span className="text-xs font-medium text-gray-500 leading-tight">
                {title}
              </span>
            </div>
            <div className="text-lg sm:text-xl font-bold text-gray-800 tabular-nums pl-0.5">
              {value}
            </div>
            {subtitle && (
              <div className="text-xxs text-gray-400 pl-0.5">{subtitle}</div>
            )}
          </div>
          {trend && (
            <div
  ...
  return (
    <div className="flex-1 relative overflow-y-auto scrollbar-thin">
      <div className="absolute inset-0 space-y-3 sm:space-y-5 px-0.5">
        {/* ── Page Header ────────────────────────────────────────────────── */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
          <div>
            <h1 className="font-semibold xs:text-lg sm:text-xl text-gray-800">
              Analytics Dashboard
            </h1>
        ...
        {activeTab === "overview" && (
          <>
            ...
            <div
              className={clsx("grid grid-cols-2 gap-2 sm:gap-3 max-xs:hidden", {
            ...
            {isAdmin && (
              <div className="grid grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6 gap-2 sm:gap-3 max-xs:hidden">
                <MetricCard
                  icon={HiOutlineTruck}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-2 sm:gap-4">
              ...
              {/* Field vs Plant Weight — small screen (beside trend chart) */}
            ...
            {/* Field Weight vs Plant Weight — large screen (full width) */}
        ...
        {activeTab === "deploymentDetails" && (isAdmin || isVisitor) && (
          <div className={CLS.tabSection}>
            {/* Metrics — desktop */}
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 sm:gap-3 max-xs:hidden">
              <MetricCard
                      ...
                      .map((territory, index) => (
                        ...
                        >
                          ...
                          <td className={CLS.tdCell}>
                            <span
                              className={`px-2 py-0.5 rounded-full text-xxs font-medium ${completionBadge(
        ...
        {activeTab === "users" && isAdmin && (
          <div className={CLS.tabSection}>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 max-xs:hidden">
              <MetricCard
                icon={HiOutlineUsers}
                title="Total Users"
                value={
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2 sm:gap-3 max-xs:hidden">
              <MetricCard
                icon={HiOutlineKey}
                title="Total Logins"
                value={
                      ...
                      {analytics?.subconAnalytics?.drivers?.performance?.map(
                        (driver, index) => (
                          ...
                          >
                            ...
                            <td className={CLS.tdCell}>
                              <span
                                className={`px-2 py-0.5 rounded-full text-xxs font-medium ${statusBadge(
                                  driver.status,
                                )}`}
                              >
                                {driver.status
